...
function bindEvents() {
    ...
    document.getElementById('btn-join-room').addEventListener('click', () => {
        ...
        if (firebaseReady) {
            ...
            // Local mode: บันทึกชื่อแล้วเข้า lobby ของห้องที่สร้างไว้
            if (State.roomCode === code) {
                // ห้องอยู่ในเครื่องนี้ — เพิ่มผู้เล่นเข้าไป
                if (State.players.some(p => p.name === name)) {
                    showToast('ชื่อนี้มีในห้องแล้ว', 'error'); return;
                }
                if (State.players.length >= State.maxPlayers) {
                    showToast('ห้องเต็มแล้ว', 'error'); return;
                }
                State.players.push({ name, avatar: AVATARS[State.players.length % AVATARS.length], id: generateId() });
                updatePlayersUI();
                showScreen('lobby');
                showToast(`${name} เข้าร่วมห้องสำเร็จ!`, 'success');
            } else {
                showToast('ไม่พบห้องนี้ — ในโหมด Local ต้องสร้างห้องก่อนจากเครื่องนี้', 'error');
            }
        }
    });
